...
def _check_tcp(host, port, timeout):
    """
    Mede apenas o tempo de abertura da conexão TCP.
    """
...
def check_service(service, timeout):
    ...
    if tipo == "icmp":
        ...
        if not host.startswith(("http://", "https://")):
            protocolo = "https" if tipo == "https" else "http"
            porta = f":{service.porta}" if service.porta else ""
            host = f"{protocolo}://{host}{porta}"
    ...
    with app.app_context():
        ...
        for service in services:
            ...
            if tempo_ms is not None:
                service.ping = round(tempo_ms, 2)
            else:
                service.ping = None

            # Horário local do Windows
            service.ultima_verificacao = datetime.now()
